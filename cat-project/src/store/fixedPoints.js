const fixedPoints = {
    US: [
        { latitude: 40.7128, longitude: -74.0060 }, // Nueva York
        { latitude: 34.0522, longitude: -118.2437 }, // Los Ángeles
        { latitude: 41.8781, longitude: -87.6298 }, // Chicago
        { latitude: 29.7604, longitude: -95.3698 }, // Houston
        { latitude: 33.4484, longitude: -112.0740 }, // Phoenix
        { latitude: 39.7392, longitude: -104.9903 }, // Denver
        { latitude: 32.7157, longitude: -117.1611 }, // San Diego
        { latitude: 47.6062, longitude: -122.3321 }, // Seattle
        { latitude: 38.9072, longitude: -77.0369 }, // Washington D.C.
        { latitude: 42.3601, longitude: -71.0589 }, // Boston
        { latitude: 36.1627, longitude: -86.7816 }, // Nashville
        { latitude: 35.2271, longitude: -80.8431 }, // Charlotte
        { latitude: 30.2672, longitude: -97.7431 }, // Austin
        { latitude: 25.7617, longitude: -80.1918 }, // Miami
        { latitude: 44.9778, longitude: -93.2650 }, // Minneapolis
        { latitude: 45.5051, longitude: -122.6750 }, // Portland
        { latitude: 33.7490, longitude: -84.3880 }, // Atlanta
        { latitude: 36.1699, longitude: -115.1398 }, // Las Vegas
        { latitude: 39.9612, longitude: -82.9988 }, // Columbus
        { latitude: 37.7749, longitude: -122.4194 }, // San Francisco
        { latitude: 35.0844, longitude: -106.6504 }, // Albuquerque
        { latitude: 43.0389, longitude: -87.9065 }, // Milwaukee
        { latitude: 41.2565, longitude: -95.9345 }, // Omaha
        { latitude: 40.4406, longitude: -79.9959 }, // Pittsburgh
        { latitude: 32.7765, longitude: -79.9311 }, // Charleston (SC)
        { latitude: 30.6954, longitude: -88.0399 }, // Mobile
        { latitude: 39.1031, longitude: -84.5120 }, // Cincinnati
        { latitude: 27.9506, longitude: -82.4572 }, // Tampa 
    ],
    GB: [
        { latitude: 51.5074, longitude: -0.1278 }, // Londres
        { latitude: 53.4808, longitude: -2.2426 }, // Manchester      
        { latitude: 52.4862, longitude: -1.8904 }, // Birmingham
        { latitude: 51.4545, longitude: -2.5879 }, // Bristol
        { latitude: 57.1497, longitude: -2.0943 }, // Aberdeen
        { latitude: 50.7184, longitude: -3.5339 }, // Exeter
        { latitude: 52.1917, longitude: -1.7073 }, // Stratford
        { latitude: 55.9533, longitude: -3.1883 }, // Edimburgo
    ],
    TH: [
        { latitude: 13.7563, longitude: 100.5018 }, // Bangkok
        { latitude: 18.7883, longitude: 98.9853 }, // Chiang Mai      
        { latitude: 8.44762, longitude: 98.9139 }, // Phuket
        { latitude: 16.4419, longitude: 102.8350 }, // Khon Kaen
    ],
    RU: [
        { latitude: 55.7558, longitude: 37.6173 }, 
        { latitude: 59.9311, longitude: 30.3609 },      
        { latitude: 55.0084, longitude: 82.9357 }, 
        { latitude: 56.0153, longitude: 92.8932 }, 

    ],
    FR: [
         { latitude: 48.8566, longitude: 2.3522 }, 
         { latitude: 45.7640, longitude: 4.8357 }, 
    ],

    TR: [
         { latitude: 39.9208, longitude: 32.8541 }, 
         { latitude: 40.54365, longitude: 29.7980 }, 
    ],
    MM: [
         { latitude: 16.8409, longitude: 96.1735 }, 
         { latitude: 21.9162, longitude: 96.0891 }, 
    ],
    CA: [
         { latitude: 57.6644, longitude: -119.3035 }, 
         { latitude: 52.8220, longitude: -106.6517 }, 
         { latitude: 53.4760, longitude: -92.6911 }, 
    ],
    EG: [
         { latitude: 29.42789, longitude: 27.0203 }, 
         { latitude: 26.2589, longitude: 31.8993 }, 
         { latitude: 23.7324, longitude: 29.9079 }, 
    ],
    


};
export default fixedPoints;